...
private const val RING_SPEEX_MODE = 1 // wideband
private const val RING_SPEEX_SAMPLE_RATE = 16_000
private const val RING_SPEEX_FRAME_SIZE = 320
...
class RingSoundBridge(
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            "scanRings" -> scanRings(call, result)
    ...
    private fun scanRings(call: MethodCall, result: MethodChannel.Result) {
        val durationMs = (call.argument<Number>("durationMs") ?: 8_000).toLong()
        val namePrefix = (call.argument<String>("namePrefix") ?: "ring").lowercase()
            ...
            try {
                ...
                val callback = object : ScanCallback() {
                    override fun onScanResult(callbackType: Int, scanResult: ScanResult) {
                        ...
                        val name = deviceName(scanResult) ?: return
                        if (!name.lowercase().startsWith(namePrefix)) return
                        val address = device.address ?: return
                        found[address.uppercase()] = mapOf(
                            "name" to name,
                            "address" to address,
                ...
                main.post { result.success(found.values.toList()) }
    ...
    @SuppressLint("MissingPermission")
    private fun deviceName(scanResult: ScanResult): String? {
        val fromRecord = scanResult.scanRecord?.deviceName?.trim()
        if (!fromRecord.isNullOrEmpty()) return fromRecord
        val fromDevice = scanResult.device?.name?.trim()
        return if (fromDevice.isNullOrEmpty()) null else fromDevice
    }
...
private class RingBleSession(
    ...
    fun receiveNextRecording(
        ...
        return mapOf(
            ...
            "oggPath" to files.ogg.absolutePath,
            "uploadPath" to files.wav.absolutePath,
            "playPath" to files.wav.absolutePath,
            "durationSec" to files.durationSec,
            "byteLength" to files.wav.length(),
    ...
    private fun saveRecording(fileIndex: Int, raw: ByteArray): SavedRingFiles {
        ...
        val pcm = decodeSpeexPackets(packets)
        ...
        val wavFile = File(dir, "ring_${fileIndex}_$stamp.wav")
        rawFile.writeBytes(raw)
        oggFile.writeBytes(ogg)
        writeWavFile(wavFile, pcm, sampleRate = RING_SPEEX_SAMPLE_RATE)
        val durationSec = max(1, ceil(packets.size * 20.0 / 1000.0).toInt())
        return SavedRingFiles(rawFile, oggFile, wavFile, durationSec, packets.size)
    }
...
private fun decodeSpeexPackets(packets: List<ByteArray>): ByteArray {
    val decoder = org.xiph.speex.SpeexDecoder()
    decoder.init(RING_SPEEX_MODE, RING_SPEEX_SAMPLE_RATE, 1, true)
    val pcm = ByteArrayOutputStream(packets.size * RING_SPEEX_FRAME_SIZE * 2)
    for (packet in packets) {
        decoder.processData(packet, 0, packet.size)
        val size = decoder.processedDataByteSize
        if (size <= 0) continue
        val frame = ByteArray(size)
        decoder.getProcessedData(frame, 0)
        pcm.write(frame)
    }
    val bytes = pcm.toByteArray()
    if (bytes.isEmpty()) {
        throw IllegalStateException("Speex decode produced empty PCM.")
    }
    return bytes
}
...
private fun buildOggSpeex(packets: List<ByteArray>): ByteArray {
    ...
    val frameSize = RING_SPEEX_FRAME_SIZE
...
private fun speexHeader(): ByteArray {
    ...
    val ints = intArrayOf(
        1,
        80,
        RING_SPEEX_SAMPLE_RATE,
        1,
        4,
        1,
        -1,
        RING_SPEEX_FRAME_SIZE,
        0,
        1,
        0,
        0,
